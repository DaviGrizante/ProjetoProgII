...
"     (@           `--------`\n"
    ...
    while(inim.pv > 0 && p->pv > 0){
        if(p->agilidade >= inim.agilidade && jaAtacou == false){
            printf("\nVocê está em combate contra %s, deseja atacar ou defender? *Veja status com \"status\"\n", inim.nome);
            fflush(stdin);
            ...
            if(strncmp(acao, "atacar", 6) == 0){
                ...
                printf("Você defendeu, aumentando sua defesa em %.2f pontos\n", defesaAumentada);
    ...
    printf("Combate encerrado. Voltando à exploração...\n");
    fflush(stdin);
    getch();
    p->defesa = originalDef;
...
void fase1(tipoPersonagem p, int saveSlot){
    ...
    if(escolhas[0] == 'n'){
        printf("\nMuito bem! Aqui você encontrará uma explicação breve sobre como jogar o jogo.\n");
        getch();
        printf("Jogos em estilo Adventure são jogos de RPG (Role-Playing Games) em formato textual, muito comum em jogos antigos de PC.\n");
        getch();
        printf("O jogo funciona com um sistema de descrições, todo o cenário, itens, inimigos, etc. Serão descritos textualmente.\n");
        getch();
        printf("As descrições são importantes para que você possa saber quais locais pode se movimentar e quando pode atacar, por exemplo.\n");
        getch();
        ...
        getch();
        printf("Agora o jogo irá começar de verdade.\n");
        getch();
    }
    while(!movimentoValido){
        printf("\nAo adentrar a floresta, você se vê cercado pelo emaranhado de árvores a sua volta. Conforme se aprofunda na inóspita região, percebe que se torna cada vez  mais difícil de identificar um caminho claro para seguir.\n");
        getch();
        printf("Apenas dois caminhos parecem se fazer presentes a você no momento, um deles seguindo ao norte e outro seguindo a leste, qual deseja seguir?\n");
        fflush(stdin);
        fgets(escolhas, TAM_ESCOLHA, stdin);
        strlwr(escolhas);
        if(strncmp(escolhas, "norte", 5) == 0){
            while(!movimentoValido){
                printf("\nVocê adentra mais a floresta, avistando uma pequena fogueira ainda acesa no chão com alguns equipamentos largados próximos a ela, tais \n"
                       "equipamentos são armas, escudos e alguns alimentos. Provavelmente pertenciam a cavaleiros que também estavam em busca da princesa.\n");
                getch();
                printf("A oeste você vê uma trilha de sangue suave, não parece ter havido muita resistência por parte dos cavaleiros. Ao norte a trilha segue normalmente.\n");
                getch();
                ...
                if(strncmp(escolhas, "oeste", 5) == 0 && inimigosDerrotados[0] == false){
                    printf("\nVocê segue a trilha de sangue até chegar a um beco sem saída envolto pelas árvores.\n");
                    getch();
                    printf("Acima de sua cabeça, você nota diversos corpos de pessoas presos ao que parecem ser teias gigantes de aranha.\n");
                    getch();
                    printf("Aos poucos diversas aranhas começam a descer pelas árvores em sua direção, as aranhas pequenas tentam te atacar, mas você consegue eliminá-las com facilidade.\n");
                    getch();
                    printf("De repente, você escuta um alto som saindo detrás das árvores. Uma aranha gigante, aparantemente a mãe das aranhas, surge em sua frente berrando de dor por você estar matando suas crias.\n");
                    getch();
                    printf("Não te resta escolha senão lutar!\n\n");
                    getch();
                    tipoInimigo aranha;
                    aranha.pv = 70;
                    aranha.forca = 60;
                    ...
                    printf("\nAo derrotar a aranha, você vê uma espada brilhante cair em sua frente, vinda de um dos corpos presos nas árvores.\n");
                    getch();
                    ...
                    printf("Você já derrotou a aranha que havia ali, não há mais nada lá para você.\n");
                    continue;
                }
                    ...
                    printf("Movimentação inválida! Digite novamente\n");
                    continue;
                }
            }
        }
        else if(strncmp(escolhas, "leste", 5) == 0 && inimigosDerrotados[1] == false){
            printf("\nSeguindo ao leste, você começa a escutar baixos sons semelhantes a uma conversa, porém em um idioma desconhecido por você.\n");
            getch();
            printf("Seguindo ainda mais ao leste, você se depara com dois goblins sentados em meio a estrada conversando.\n");
            getch();
            printf("Caido, ao lado deles, está um homem humano aparentemente sem vida, seu peito sangra bastante, provavelmente era um cavaleiro igual você que morreu para os goblins.\n");
            getch();
            ...
            if(strncmp(escolhas, "lutar", 5) == 0){
                printf("Você dispara em direção aos goblins com sua arma em punhos.\n");
                tipoInimigo goblins;
                ...
                printf("\nAo derrotar os goblins, você percebe que o homem carregava consigo um escudo muito melhor que o seu.\n");
                getch();
                ...
                printf("Você decide evitar a luta contra os goblins e volta furtivamente de onde veio.\n");
                getch();
                continue;
            }
            else if(strncmp(escolhas, "status", 6) == 0){
            ...
            printf("Já derrotei os goblins que estavam ali, não há mais nada para fazer lá.\n");
            sleep(1);
            continue;
        }
        else if(strncmp(escolhas, "status", 6) == 0){
...
"                   '''\n"
    ...
    getch();
    printf("Um exército de monstros de todos os tipos se juntaram e planejaram um ataque coordenado contra o castelo.\n");
    getch();
    printf("O ataque ocorreu um dia antes do maior festival do reino, o festival de comemoração da maioridade da princesa.\n");
    getch();
    printf("Durante as duas semanas antes da festa, diversos cavaleiros importantes do reino se aventuravam em incursões para buscar pelas iguarias mais refinadas ao redor do mundo.\n");
    getch();
    printf("Nesse momento, com os principais cavaleiros fora, os monstros se aproveitaram e atacaram o reino, raptando a princesa e a tornando refém deles.\n");
    getch();
    printf("Você, como cavaleiro do reino, deve ir atrás da princesa e resgatá-la das garras desses monstros vis.\n");
    getch();
    printf("Suas investigações te levaram primeiro a floresta perdida, um local dominado pelos mais terríveis monstros existentes.\n");
    getch();
    printf("Será que você conseguirá resistir a seus ataques e salvar a princesa? Seja forte e prepare-se para a jornada!!!\n");
    p.fase = 1;
    getch();
    salvarPersonagem(p,saveSlot, true);
}
...
void criarPersonagem(int saveSlot){
    ...
    while(true){
        ...
        printf("Qual será o sexo do seu personagem? M/F\n");
        fflush(stdin);
        scanf("%c", &novo.sexo);
        novo.sexo = tolower(novo.sexo);
        if(novo.sexo == 'm'){
            printf("Seu personagem é do sexo masculino!\n");
        }
        else if(novo.sexo == 'f'){
            printf("Seu personagem é do sexo feminino!\n");
        }
        else{
        ...
        do{
            char retry;
            printf("\nQual será a classe do seu personagem? G/L/B\n");
            printf("*Caso não conheça as classes, digite \'h\' no console.\n");
            fflush(stdin);
            scanf("%c", &novo.classe);
            ...
            if(novo.classe == 'h'){
                printf("\nO caracter \'g\' corresponde ao guerreiro, focado em defesa alta, ataque mediano e destreza baixa.\n"
                    "\nO caracter \'l\' corresponde ao ladrão, focado em destreza alta, defesa mediana e ataque baixo.\n"
                    "\nO caracter \'b\' corresponde ao bárbaro, focado em ataque alto, destreza mediana e defesa baixa.\n"
                );
                printf("\nQual dessas classes você prefere?\n");
                fflush(stdin);
                scanf("%c", &novo.classe);
                novo.classe = tolower(novo.classe);
            }
            ...
            printf("Você deseja jogar com essa classe? S/N\n");
            fflush(stdin);
            scanf("%c", &retry);
            retry = tolower(retry);
            if(retry == 's'){
                ...
                printf("Voltando a seleção de classe...");
                sleep(2);
                novo.classe = 'x';
            }
        }while(novo.classe == 'x');
...
void verificarSave(int saveSlot, bool newGame){
    ...
    if(p.isAlive == false && newGame == true){
        printf("Redirecionando para a criação de personagem...\n");
        sleep(1);
        ...
        printf("Impossível carregar save, escolha um arquivo de salvamento com um personagem disponível para carregar o jogo.\n");
        sleep(3);
        menu();
    }
}
...
void selecaoSave(bool newGame){
    ...
    int saveFile = 0;
    system("cls");
    printf("Qual arquivo de salvamento você vai utilizar?\n");
...
" \\___|_|  \\___|\\__,_|_|\\__|___/\n"
    ...
    sleep(2);
    printf("Davi de Sá Grizante\n");
    sleep(2);
    printf("Kelwin Regis Esechiel\n");
    sleep(2);
    printf("\n-------------------------------------------------------CRÉDITOS DAS ARTES ASCII-------------------------------------------------------\n\n");
    sleep(2);
...
"-~--~                   ~---__|,--~'                  ~~----_____-~'   `~----~~\n"
    ...
    printf("Escolha uma das opções acima com o número correspondente: \n");
    fflush(stdin);
    scanf("%d", &opcoes);
    ...
    switch(opcoes){
        ...
        case 4:
            ...
            exit(0);
        default:
            printf("Comando não detectado! Tente novamente.\n");
            sleep(2);
            continue;
        }
    }
